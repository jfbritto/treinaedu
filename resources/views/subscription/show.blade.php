<x-layout.app title="Minha Assinatura">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center gap-1">
                                            <input type="text" name="expiry_month" id="upd_expiry_month" required placeholder="MM" maxlength="2"
                                                ...
                                                @input="
                                                    ...
                                                    if (v.length === 2) document.getElementById('upd_expiry_year').focus();
                                                ">
                                            <span class="text-gray-300 text-lg font-light">/</span>
                                            <input type="text" name="expiry_year" id="upd_expiry_year" required placeholder="AAAA" maxlength="4"
                                                ...
                                                @input="
                                                    let v = $event.target.value.replace(/\D/g,'');
                                                    $event.target.value = v;
                                                    if (v.length === 4) document.getElementById('upd_ccv_field').focus();
                                                "
                                                @keydown.backspace="if (!$event.target.value) document.getElementById('upd_expiry_month').focus()">
                                        </div>
                                        <p class="text-xs text-gray-400 mt-1">Ex: 12/2028</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 mb-1">CVV</label>
                                        <div class="relative">
                                            <input type="password" name="ccv" id="upd_ccv_field" required placeholder="•••" maxlength="4"
                                                class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
    ...
    @if($subscription && $subscription->status !== 'cancelled')
        <div class="mt-6 pt-6 border-t border-gray-200" x-data="{ step: 0, reason: '' }">

            {{-- Step 0: Link discreto --}}
            <div x-show="step === 0" class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500">Deseja cancelar sua assinatura?</p>
                    <p class="text-xs text-gray-400">Você mantém acesso até o fim do período atual.</p>
                </div>
                <button @click="step = 1" type="button" class="text-xs font-medium text-red-500 hover:text-red-700 transition">
                    Cancelar assinatura
                </button>
            </div>

            {{-- Step 1: Motivo --}}
            <div x-show="step === 1" x-cloak x-transition class="bg-white rounded-xl border border-gray-200 p-6">
                <h3 class="text-sm font-semibold text-gray-800 mb-1">Sentimos muito que você queira sair</h3>
                <p class="text-xs text-gray-500 mb-4">Nos ajude a melhorar — qual o principal motivo?</p>

                <div class="space-y-2 mb-5">
                    @php
                        $reasons = [
                            'expensive' => 'O preço está alto para minha empresa',
                            'not_using' => 'Não estamos usando o suficiente',
                            'missing_features' => 'Falta alguma funcionalidade que preciso',
                            'switching' => 'Vou usar outra plataforma',
                            'temporary' => 'Pausa temporária, pretendo voltar',
                            'other' => 'Outro motivo',
                        ];
                    @endphp
                    @foreach($reasons as $value => $label)
                        <label @click="reason = '{{ $value }}'" :class="reason === '{{ $value }}' ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-gray-300'"
                               class="flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition">
                            <div :class="reason === '{{ $value }}' ? 'border-primary' : 'border-gray-300'"
                                 class="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0">
                                <div x-show="reason === '{{ $value }}'" class="w-2 h-2 rounded-full bg-primary"></div>
                            </div>
                ...
                <div class="flex items-center gap-3">
                    <button @click="if(reason) step = 2" :disabled="!reason" :class="!reason ? 'opacity-50 cursor-not-allowed' : ''"
                        class="inline-flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-semibold transition">
                        Continuar cancelamento
                    </button>
                    <button @click="step = 0; reason = ''" type="button" class="text-sm text-gray-500 hover:text-gray-700 transition">
                        Mudei de ideia
                    </button>
                </div>
            </div>
            ...
            {{-- Step 2: Retenção --}}
            <div x-show="step === 2" x-cloak x-transition class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="text-center mb-6">
                    ...
                    <h3 class="text-lg font-bold text-gray-800 mb-1">Tem certeza?</h3>
                    <p class="text-sm text-gray-500">Ao cancelar, você e sua equipe perderão acesso a:</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                    <div class="flex items-center gap-2 p-3 bg-red-50 rounded-lg text-sm">
                        ...
                        <span class="text-red-700">{{ $usage['users'] }} usuários cadastrados</span>
                    </div>
                    <div class="flex items-center gap-2 p-3 bg-red-50 rounded-lg text-sm">
                        <svg class="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        <span class="text-red-700">{{ $usage['trainings'] }} treinamentos criados</span>
                    </div>
                    <div class="flex items-center gap-2 p-3 bg-red-50 rounded-lg text-sm">
                        <svg class="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        <span class="text-red-700">{{ $usage['certificates'] }} certificados emitidos</span>
                    </div>
                    <div class="flex items-center gap-2 p-3 bg-red-50 rounded-lg text-sm">
                        <svg class="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        <span class="text-red-700">Todo o progresso dos colaboradores</span>
                    </div>
                </div>

                <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                    <p class="text-sm font-semibold text-green-800 mb-1">Seus dados ficam salvos por 30 dias</p>
                    <p class="text-xs text-green-700">Se mudar de ideia, basta assinar novamente e tudo estará exatamente como deixou.</p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button @click="step = 0; reason = ''" type="button"
                        class="inline-flex items-center gap-2 text-white px-5 py-2 rounded-lg text-sm font-semibold transition shadow-sm"
                        style="background: linear-gradient(135deg, var(--primary), var(--secondary))">
