<x-layout.app title="Editar Grupo">
    ...
    <form method="POST" action="{{ route('groups.update', $group) }}"
          x-data="{
              name: '{{ old('name', addslashes($group->name)) }}',
              description: '{{ old('description', addslashes($group->description ?? '')) }}',
              search: '',
              selected: @js(old('users', $groupUserIds)),
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 sticky top-6">
                    ...
                    <div class="pt-4 space-y-3">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-800">{{ $group->created_at->format('d/m/Y') }}</span>
                        </div>
                    </div>
